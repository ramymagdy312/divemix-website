...
const FolderExplorer: React.FC<FolderExplorerProps> = ({
  ...
  const handleFileSelect = async (files: FileList | null) => {
    ...
    try {
      ...
      toast.success(`Successfully uploaded ${uploadedImages.length} image(s) to ${folderName}`);
      loadCurrentPath(); // Refresh to show new images
    } catch (error) {
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {serverImages.map((imageUrl, index) => {
                    const isSelected = images.includes(imageUrl);
                    return (
                      <div
                        key={index}
