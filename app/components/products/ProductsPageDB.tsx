...
export default function ProductsPageDB({ children, data: initialData }: ProductsPageDBProps) {
  ...
  useEffect(() => {
    ...
    const fetchProductsPageData = async () => {
      try {
        const { data: pageData, error } = await supabase.from('products_page').select('*').single();
        if (error) {
