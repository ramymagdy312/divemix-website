  ...
  const fixAllIssues = async () => {
    ...
    try {
      ...
      if (testError) {
        setStatus(
          (prev) =>
            prev + `❌ Supabase connection failed: ${testError.message}\n`
        );
        setStatus(
          (prev) => prev + "⚠️ Running in development mode with mock data\n"
        );
      } else {
      ...
      setStatus(
        (prev) => prev + "2. Creating/updating database functions...\n"
      );
      try {
        ...
        setStatus(
          (prev) => prev + `⚠️ Function creation skipped: ${error.message}\n`
        );
      }
      ...
      for (const table of tables) {
        try {
          ...
          if (error) {
            setStatus(
              (prev) =>
                prev + `❌ Table ${table} has issues: ${error.message}\n`
            );
          } else {
          ...
          setStatus(
            (prev) => prev + `❌ Table ${table} error: ${error.message}\n`
          );
        }
      }
      ...
      try {
        ...
        if (catError) {
          setStatus(
            (prev) =>
              prev + `❌ Product categories error: ${catError.message}\n`
          );
        } else {
          setStatus(
            (prev) =>
              prev + `✅ Product categories: ${categories?.length || 0} found\n`
          );
        }
      } catch (error: any) {
        setStatus(
          (prev) =>
            prev + `❌ Product categories test failed: ${error.message}\n`
        );
      }
      ...
      try {
        ...
        if (servError) {
          setStatus(
            (prev) => prev + `❌ Services error: ${servError.message}\n`
          );
        } else {
          setStatus(
            (prev) => prev + `✅ Services: ${services?.length || 0} found\n`
          );
        }
      } catch (error: any) {
        setStatus(
          (prev) => prev + `❌ Services test failed: ${error.message}\n`
        );
      }
      ...
      try {
        ...
        if (appError) {
          setStatus(
            (prev) => prev + `❌ Applications error: ${appError.message}\n`
          );
        } else {
          setStatus(
            (prev) =>
              prev + `✅ Applications: ${applications?.length || 0} found\n`
          );
        }
      } catch (error: any) {
        setStatus(
          (prev) => prev + `❌ Applications test failed: ${error.message}\n`
        );
      }
      ...
      try {
        ...
        if (galError) {
          ...
          setStatus(
            (prev) =>
              prev + `✅ Gallery images: ${gallery?.length || 0} found\n`
          );
        }
      } catch (error: any) {
        setStatus(
          (prev) => prev + `❌ Gallery test failed: ${error.message}\n`
        );
      }
      ...
      for (const pageType of pageTypes) {
        try {
          ...
          if (pageError) {
            setStatus(
              (prev) => prev + `❌ ${pageType} error: ${pageError.message}\n`
            );
          } else {
          ...
          setStatus(
            (prev) => prev + `❌ ${pageType} test failed: ${error.message}\n`
          );
        }
      }
      ...
      if (!supabaseUrl || !supabaseKey) {
        ...
        setStatus(
          (prev) =>
            prev + "❌ Environment variables contain placeholder values\n"
        );
      } else {
        setStatus((prev) => prev + "✅ Environment variables configured\n");
        setStatus((prev) => prev + `   URL: ${supabaseUrl}\n`);
        setStatus(
          (prev) => prev + `   Key: ${supabaseKey.substring(0, 20)}...\n`
        );
      }
      ...
      if (componentIssues.length > 0) {
        setStatus(
          (prev) => prev + "❌ Component compatibility issues found:\n"
        );
        ...
        setStatus(
          (prev) => prev + "✅ All components compatible with database schema\n"
        );
      }
      ...
      setStatus((prev) => prev + "\n🚀 NEXT STEPS:\n");
      setStatus(
        (prev) => prev + "   1. Visit /setup-all-db if tables are empty\n"
      );
      setStatus(
        (prev) =>
          prev +
          "   2. Test each page: /products, /services, /applications, /gallery\n"
      );
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-6">
        🔧 Project Issue Diagnostics & Fixes
      </h1>
